...
export async function wellKnownLookup(baseUrl: string): Promise<string> {
    ...
    if (!result.ok) throw new Error('The well known lookup didn\'t respond with 200!')
    if (!data?.['m.homeserver']?.base_url) throw new Error('The well known lookup didn\'t include `m.homeserver`.base_url!')
